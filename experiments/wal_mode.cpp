// g++ wal_mode.cpp ../sqlite3.o -ldl -lpthread
...
int main(int argc, char** argv) {
    ...
    // Enable WAL mode
    rc = sqlite3_exec(sqldb, "PRAGMA journal_mode = WAL;", 0, 0, 0);
    if (rc != SQLITE_OK) {
        cout << "Failed setting journal_mode to WAL" << endl;
        goto out;
    }

    // Set synchronous to FULL, for the WAL to be synced on every commit
    rc = sqlite3_exec(sqldb, "PRAGMA synchronous = FULL;", 0, 0, 0);
    if (rc != SQLITE_OK) {
        cout << "Failed setting synchronous to FULL" << endl;
        goto out;
    }

    // Set locking mode to EXCLUSIVE
    rc = sqlite3_exec(sqldb, "PRAGMA locking_mode = EXCLUSIVE;", 0, 0, 0);
    if (rc != SQLITE_OK) {
        cout << "Failed setting locking_mode to exclusive, rc: " << rc << endl;
        goto out;
    }
